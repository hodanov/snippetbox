...
func TestShowSnippet(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid ID", "/snippet/1", http.StatusOK, []byte("An old silent pod...")},
...
func TestSignupUser(t *testing.T) {
	// The application struct containing our mocked dependencies and set
	// up the test server for running and end-to-end test.
	app := newTestApplication(t)
	ts := newTestServer(t, app.routes())
	defer ts.Close()

	// Make a GET /user/signup request and then extract the CSRF token from
	// the response body.
	...
	tests := []struct {
		...
		{"Empty name", "", "bob@example.com", "validPa$$word", csrfToken, http.StatusOK, []byte("This field can&#39;t be blank")},
		{"Empty email", "Bob", "", "validPa$$word", csrfToken, http.StatusOK, []byte("This field can&#39;t be blank")},
		{"Empty password", "Bob", "bob@example.com", "", csrfToken, http.StatusOK, []byte("This field can&#39;t be blank")},
