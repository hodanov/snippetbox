...
func TestUserModelGet(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Initialize a connection pool to the test DB, and defer a
			...
			// To check for equality between arbitrarily complex custom types.
			if !reflect.DeepEqual(user, tt.wantUser) {
				t.Errorf("want %v; got %v", tt.wantUser, err)
			}
		})
	}
}
