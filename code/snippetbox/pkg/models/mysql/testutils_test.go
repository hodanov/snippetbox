...
func newTestDB(t *testing.T) (*sql.DB, func()) {
	// Establish a sql.DB connection pool for test database. Because the
	...
	// in one `db.Exec()` call.
	db, err := sql.Open("mysql", "test_web:pass@tcp(mysql:3306)/test_snippetbox?parseTime=true&multiStatements=true")
	if err != nil {
		t.Fatal(err)
	}
